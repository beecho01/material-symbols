...
app.controller("AppCtrl",["$scope","$http","$mdToast",function($scope, $http, $mdToast) {
    ...
    $scope.parseURLParams = function () {
      ...
      $scope.searchTerm = params.get("search") ? params.get("search") : "";

      if (params.get("library")) {
        console.log("loading external library");

        $scope.externalLibrary = params.get("library");
        $scope.activeExternalLibrary = $scope.supportedLibraries.find(
          (lib) => lib.url == $scope.externalLibrary
        );
    ...
    $scope.copyToClipboard = function (content, msg) {
      ...
      $scope.showToast(
        typeof msg === "undefined" ? "Copied to clipboard" : msg
      );
    };

    $scope.showToast = function (message) {
      $mdToast
        .show($mdToast.simple().textContent(message).hideDelay(3000))
        .then(function () {
          // Toast dismissed.
        })
        .catch(function () {
        ...
        for (const icon in my_list) {
          ...
          this_icon.then(function (my_icon) {
            ...
            let keywords = my_list[icon].keywords,
              aliases = keywords ? keywords.join(", ") : "";
